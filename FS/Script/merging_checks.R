...
#rename 
DR1_FS <- all_dr1_bind %>% 
  rename("CiN_started" = 'Number of CIN plans that started this month in the LA',
         "CiN_started_excl_CPP" = 'Number of CIN plans that started this month in the LA (exc CPP)',
         "CiN_open"= 'Number of open CIN cases this month in the LA',
...
DR1_FS %>% 
  group_by(Month, LA) %>% 
  select(CiN_open) %>% 
  filter(CiN_open == "NA" & CiN_open_all == !"NA") %>% 
  table() 
